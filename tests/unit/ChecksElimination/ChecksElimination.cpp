...
TEST(ChecksElimination, boundscheck_0) {
  FunctionType FFnTy(ValueType::Int32, {ValueType::Int32, ValueType::Int32});
  Function F("F", FFnTy);
  ...
  auto Bounds1 = F.create<ConstantNode>(ValueType::Int32, 200);
  auto firstCheck = F.create<CallBuiltinNode>(
      F_N0, "boundsCheck", ValueType::Void, std::vector<Node *>{F_Use3, Bounds1});
  auto FirstCheckReg = F.create<RegionNode>();
  FirstCheckReg->addCFInput(firstCheck);
  ...
  // block 5
  // bound is dominated and fully redundant by previous check
  auto Bounds2 = F.create<ConstantNode>(ValueType::Int32, 100);
  auto F_B5_0 = F.create<RegionNode>();
  F_B5_0->addCFInput(F_ifFalse);
  auto secondCheck = F.create<CallBuiltinNode>(
      F_B5_0, "boundsCheck", ValueType::Void, std::vector<Node *>{F_Use3, Bounds2});
